package com.mobileoid2.samplelibrary.Dialog;
...
import com.mobileoid2.samplelibrary.AppUtils;
import com.mobileoid2.samplelibrary.R;
...
public enum DialogsCustom {
    ...
    public interface onSubmit {
        void onSubmitClicked(String todo);
    }

    public void showPermissionsDialog(final Activity activity, ArrayList<BeanDialogsPermissions> permissions, View.OnClickListener actionListner) {
        try {
    ...
    public void showEditBoxDialog(final Activity activity, onSubmit onSubmit, String title) {
        try {
            if (dialog != null) {
                if (dialog.isShowing()) dialog.dismiss();
                dialog = null;
            ...
            final onSubmit thisOnSubmit = onSubmit;
            dialog = new Dialog(activity);
            dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
            dialog.setContentView(R.layout.layout_dialog_editbox);
            dialog.getWindow().getAttributes().windowAnimations = R.style.DialogThemeNoInternet;
            dialog.getWindow().getAttributes().gravity = Gravity.CENTER;
            dialog.getWindow().setBackgroundDrawable(activity.getResources().getDrawable(R.drawable.shape_layout_white_solid));
            dialog.setCancelable(true);
            ...
            dialog.findViewById(R.id.button_okay).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    dialog.dismiss();
                    if (((EditText) dialog.findViewById(R.id.edittext_input)).getText().toString().equals(""))
                        Toast.makeText(activity, "Please enter some value", Toast.LENGTH_SHORT).show();
                    else
                        thisOnSubmit.onSubmitClicked(((EditText) dialog.findViewById(R.id.edittext_input)).getText().toString());
                }
            });
